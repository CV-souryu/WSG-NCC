...
`region` 的 `BOTTOM` 是激活区域的下边界，应按照画面里底部 UI / 文字区实际占用的
比例来设置：底部 UI 占得多就调小（如 40–60），占得少可以调大（如 80）。同一全码本
无需重建，推理时直接传 `--region` 即可。

58 张主测试卡（`region` 不同、其余参数相同）实测对比：

| region | 最低 top-1 | 中位 top-1 | ≥0.9 | 不同船最小分差 |
|---|---:|---:|---:|---:|
| `(0, 60, 0, 100)` | 0.9020 | 0.9663 | 58/58 | 0.1684 |
| `(0, 80, 0, 100)` | 0.8389 | 0.9599 | 55/58 | 0.2525 |

两组在默认 `min_confidence=0.7` 下都 58/58 通过。60% 绝对分数更漂亮、更保守；
80% 绝对分数略降，但对“不同船”的区分余量更大。请根据下方 UI 的占比自行调整
激活扫描区域。
